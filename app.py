...
def add_student():
    ...
    new_student = Student(
        student_id=student_id, name=name, score=int(score)
    )
...
def profile(student_id):
    ...
    history = (
        ScoreHistory.query.filter_by(student_id=student_id)
    ...
    return render_template(
        ...
        student=student,
        history=history,
        current_admin=current_user.username,  # Passes logged-in admin's username to Jinja
    )
...
def delete_score(history_id):
    ...
    # Authorization Check: Enforce that only creator can delete
    if history_item.admin_username != current_user.username:
        return jsonify(
            {"success": False, "message": "Unauthorized to delete this score"}
        ), 403

    student = Student.query.filter_by(
        student_id=history_item.student_id
    ).first()
    if student:
        ...
        return jsonify(
            {"success": True, "message": "Score rolled back successfully!"}
        )
